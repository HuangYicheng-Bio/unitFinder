...
def getDistenseBlock(pwd2,pwd3,name,start,end,Chr,all1,covpwd,pwdu1,pwdu4):
	...
	for k,v in dict1.items():
		...
		if len(len1)!=0:
			...
			for i in list2:
				...
				#list2t.append([i[0],np.mean([i[1],i[2]])])
				list2t.append([i[0],i[1]])
				list2t.append([i[0],i[1]])
				#ax[2-z].plot(t, lenp,color=matplotlib.colors.to_hex(colorR),alpha=float(len(list2)/maxn))

			print (min(lociD),0,max(lociD)-min(lociD),max(t0)-0,gseq.description,len(v))
			#ymax=25000
			#ax[2-z].set(ylim=(0, ymax))
			#wth=(end-start)/50
			#ax1=inset_axes(ax[2-z],width="100%", height="100%",bbox_to_anchor=((max(lociD)+min(lociD))/2-wth,0,2*wth,ymax),bbox_transform=ax[2-z].transData,loc=2,borderpad=0)
			#ax1.set(ylim=(0, ymax))
			#ax1.tick_params(axis='both', which='both', labelsize='large',bottom=False, top=False, labelbottom=False,left=False, right=False, labelleft=False)
			#bplot=ax1.boxplot(t0,showfliers=False,patch_artist=True,medianprops={"color": "red", "linewidth": 0.5},boxprops={"facecolor": colorR, "edgecolor": colorR,"linewidth": 0.5},whiskerprops={"color": colorR, "linewidth": 0.5},capprops={"color": colorR, "linewidth": 0.5})
			#ax[2-z].plot(lociDl,tl0,color="black",lw=0.5)
			wth=(end-start)/50
			bplot=ax[2-z].boxplot(t0,showfliers=False,patch_artist=True,medianprops={"color": "red", "linewidth": 0.5},boxprops={"facecolor": colorR, "edgecolor":"black","linewidth": 0.5},whiskerprops={"color": 'black', "linewidth": 0.5},capprops={"color": 'black', "linewidth": 0.5},widths=wth,positions=np.array([(min(lociD)+max(lociD))/2.0]))
			boxt.append(t0)
			boxc.append(colorR)
		...
		#			lenp0.append(np.mean(lenp))
			...
			#	lenp0.append(np.mean(lenp))
				...
				#print (float(len(list2)/maxn))
			ax[2-z].scatter(t0,lenp0,color=matplotlib.colors.to_hex(colorR),s=0.5)
			#bplot=ax1.violinplot(t0,showmeans=False, showmedians=False,showextrema=False)
			#for pc in bplot['bodies']:
			#	pc.set_facecolor(colorR)
			#	pc.set_edgecolor(colorR)
			#	pc.set_alpha(1)
			#quartile1, medians, quartile3 = np.percentile(t0, [25, 50, 75])
			#ax1.scatter([1], medians, marker='o', color='red', s=30, zorder=3)
			...
			if c0>=11:
				c0=0
	#bplot=ax[2-z].boxplot(boxt,showfliers=False,patch_artist=True,medianprops={"color": "red", "linewidth": 0.5},boxprops={"facecolor": boxc, "edgecolor": boxc,"linewidth": 0.5},whiskerprops={"color": boxc, "linewidth": 0.5},capprops={"color": boxc, "linewidth": 0.5},positions=np.array(boxp),widths=100)
	#bplot=ax[2-z].boxplot(boxt,showfliers=False,patch_artist=True,positions=np.array(boxp),widths=100)
	ax[2-z].set_title('Distense within Blocks', size="small")
	file1=open(pwd3,'r')
	list1=[]
	t=[]
	tall=[]
	...
	ax[0].set_xticks(range(start, end, 500000))
